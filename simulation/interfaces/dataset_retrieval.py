...
for step, batch in enumerate(dataloader):
    ...
    print(f'Step {step}')
    batch=batch.numpy()
    batches_buffer.append(batch)
    print(batch)
    print(batch.shape)
    # flush to file every 400 steps.
    if (step+1) % 400 == 0:
        print('Flushing to file...')
        batches_buffer = np.array(batches_buffer)
        batches_file = open(f'/Users/saaketh.narayan/Desktop/yamls/shuffling_experiments/dataset_shards/2stream_pickled/batch_file_{file_num}', 'wb')
        pickle.dump(batches_buffer, batches_file)
        batches_file.close()
        batches_buffer = []
        file_num += 1
